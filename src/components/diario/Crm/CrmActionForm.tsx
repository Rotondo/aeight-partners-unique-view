...
export const CrmActionForm: React.FC<CrmActionFormProps> = ({ onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    partner_id: '',
    next_steps: '',
    next_step_date: undefined as Date | undefined
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createAcaoCrm({
        ...
        next_step_date: formData.next_step_date?.toISOString(),
        metadata: {
          created_via: 'form'
        }
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Defina os próximos passos necessários..."
              rows={2}
            />
          </div>
          ...
          {formData.next_steps && (
            <div className="space-y-2">
              ...
              <DatePicker
                selected={formData.next_step_date}
                onSelect={(date) => setFormData({...formData, next_step_date: date})}
