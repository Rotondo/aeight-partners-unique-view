...
export const useParceiroStats = (filters: UseParceiroStatsFilters = {}) => {
  ...
  // Fase 1: Filtragem de oportunidades (useMemo para performance)
  const filteredOportunidades = useMemo(() => {
    if (oportunidadesLoading) return [];
    ...
    return result;
  }, [oportunidades, filters, oportunidadesLoading]);

  // Fase 2: Cálculo das estatísticas dos parceiros (useMemo para performance)
  const [processing, setProcessing] = useState(false);
  const [stats, setStats] = useState<ParceiroStats[]>([]);

  useEffect(() => {
    if (partnersLoading || oportunidadesLoading) {
      setStats([]);
      setProcessing(false);
      return;
    }
    setProcessing(true);
    // Simula processamento pesado (pode ser removido em produção)
    const timeout = setTimeout(() => {
      // Filter partners to only include 'parceiro' type
      const parceiros = partners.filter(p => p.tipo === 'parceiro');
      const newStats = parceiros.map(parceiro => {
      ...
      setStats(newStats);
      setProcessing(false);
    }, 200); // 200ms de atraso artificial para UX
    return () => clearTimeout(timeout);
  }, [partners, filteredOportunidades, partnersLoading, oportunidadesLoading]);

  return {
    stats,
    loading: partnersLoading || oportunidadesLoading || processing
  };
};
